...
*/
compileCSS = (input, output) => {

  console.log('Compiling CSS from ' + input.replace(ROOTPATH, '') + ' to ' + output.replace(ROOTPATH, ''));
  exec('./node_modules/.bin/node-sass ' + input + ((isFolder(input)) ? ' -o ' : ' ') + output, completionHandler);
...
compileJS = () => {
  ...
  exec('./node_modules/.bin/browserify ' + ROOTPATH + '/src/js/p2s.js -o ' + ROOTPATH + '/public/js/p2s.js -t [ babelify --presets [ es2015 ] ]');
